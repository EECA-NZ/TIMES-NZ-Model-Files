...
def empirical_scrappage_percentiles(df: pd.DataFrame) -> pd.DataFrame:
    """
    Applies age-weighted split of n_fleet into tertiles and computes empirical scrappage percentiles.
    """
    out = []

    for vt, block in df.groupby("vehicle_size_group"):
        max_age = block["age"].max()
        if max_age == 0 or block["n_fleet"].sum() == 0:
            continue

        # Base share of each tertile within vehicle type
        base_share = (
            block.groupby("tertile")["n_fleet"].sum()
        ...
        age_factor = age_factor.clip(lower=0)  # prevent negatives

        weight = base_share[rank].to_numpy() * (age_factor ** rank)
        weight /= block.groupby("age")["tertile"].transform(lambda _: weight.sum())
        block["n_fleet"] *= weight

        # Compute empirical percentiles from weighted n_fleet
...
def first_year_vkt(df: pd.DataFrame) -> pd.DataFrame:
    """
    Return VKT value at first full year in fleet (age 1 preferred, fallback to age 2 if missing)
    for each (vehicle_size_group, tertile) pair.
    """
    df = df.copy()
    ...
    vkt_age = (vkt_age.sort_values("age")  # age 1 before 2
                     .drop_duplicates(subset=["vehicle_size_group", "tertile"], keep="first"))

    return (vkt_age[["vehicle_size_group", "tertile", "vkt per tert"]]
            .rename(columns={"vehicle_size_group": "vehicletype", "vkt per tert": "vkt_age_1"}))
...
def vkt_summary(df_yr: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        df_yr.groupby(["vehicle_size_group", "tertile"], as_index=False)
             .agg({
                 "n_fleet": "sum",
                 "vkt per tert": "sum",
                 "vkt_weighted": "sum"
...
# def fallback_scrappage_heavy_to_medium(df: pd.DataFrame) -> pd.DataFrame:
#     df = df.copy()
#     scrap_cols = ["scrap_p50", "scrap_p60", "scrap_p70", "scrap_p80"]

#     # Build Medium Truck reference table by tertile
#     med_ref = (
#         df[df["vehicletype"] == "Medium Truck"]
#         .set_index("tertile")[scrap_cols]
#         .rename(columns=lambda c: f"{c}_med")
#     )

#     # Merge Medium Truck values to all rows based on tertile
#     df = df.merge(med_ref, how="left", left_on="tertile", right_index=True)

#     # Replace Heavy Truck values with Medium Truck values
#     for col in scrap_cols:
#         df.loc[df["vehicletype"] == "Heavy Truck", col] = df.loc[
#             df["vehicletype"] == "Heavy Truck", f"{col}_med"
#         ]

#     # Drop temp columns
#     df = df.drop(columns=[f"{col}_med" for col in scrap_cols])
...
def main() -> None:
    ...
    cols = ["vehicletype", "tertile", "vehiclecount", "vktvalue",
            "annual_vkt_avg", "vktshare", "tertile_AFA", "vkt_age_1",
            "scrap_p50", "scrap_p60", "scrap_p70", "scrap_p80"]
   
    # Round selected columns
    for col in ["vktvalue", "annual_vkt_avg", "vktshare", "tertile_AFA",
                "vkt_age_1", "scrap_p50", "scrap_p60", "scrap_p70", "scrap_p80"]:
    ...
    logger.info(f"VKT utils summary statistics written to {out_path}")
