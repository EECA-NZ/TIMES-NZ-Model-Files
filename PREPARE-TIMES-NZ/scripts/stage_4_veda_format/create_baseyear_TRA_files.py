...
# Constants
# ────────────────────────────────────────────────────────────────
FUEL_TO_COMM = {"PET": "TRAPET", "DSL": "TRADSL", "LPG": "TRALPG",
                "ELC": "TRAELC", "H2": "TRAH2R"}
...
VAR_LIST   = ["efficiency", "life(years)", "vktvalue", "pjvalue", "ACT_BND_0", "Cap2Act", "annual_utilisation_rate", "cost_2023_nzd", "operation_cost_2023_nzd", "vehicle_count", "fuelshare"]
VAR_RENAME = {
    "efficiency":   "EFF~2023",
    "life(years)":  "LIFE",
...
def extract_tertile(tech: str) -> int:
    if tech.endswith("_LOW"):
        return 0
    elif tech.endswith("_MED"):
        return 1
    elif tech.endswith("_HIGH"):
        return 2
...
def assign_tcap(base):
    if base in {
        "T_F_DSHIPP15", "T_F_ISHIPP15", "T_O_FuelJet", "T_O_FuelJet_Int", "T_R_Rail15", "T_P_Rail15"
    }:
        return "PJa"
    elif base in {
        "T_P_CICEPET15", "T_P_CICEDSL15", "T_P_CBEVNEW15", "T_P_CBEVUSD15", "T_P_CICELPG15", "T_P_CHYBPET15", "T_P_CPHEVPET15", "T_P_CPHEVELC15",
        "T_C_CICEPET15", "T_C_CICEDSL15", "T_C_CBEVNEW15", "T_C_CICELPG15", "T_C_CHYBPET15"
    }:
        return "000cars"
    elif base in {"T_P_MICEPET15", "T_P_MBEVELC15"}:
        return "000mcy"
    elif base in {"T_P_BICEPET15", "T_P_BICEDSL15", "T_P_BICELPG15", "T_P_BBEVELC15"}:
        return "000busses"
    elif base in {
        "T_F_LTICEPET15", "T_F_LTICEDSL15", "T_F_LTBEVELC15",
        "T_F_MTICEDSL15", "T_F_MTBEVELC15",
        "T_F_HTICEDSL15", "T_F_HTBEVELC15"
    }:
        return "000trucks"
    else:
        return None
...
def load_var_tables(demand: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    for var in VAR_LIST:
        sub = (demand
               .loc[demand["variable"] == var]
               .drop(columns="variable")
               .assign(**{c: lambda d, col=c: d[col].str.strip().str.lower() for c in ("vehicletype", "fueltype", "technology", "region")}))
        tbls[var] = sub
    return tbls
...
def comm_out_for_tech(base: str) -> str | None:
    ...
    PREFIX = {
        "T_P_C": "T_P_Car",  "T_C_C": "T_C_Car",
        "T_P_M": "T_P_Mcy",  "T_P_B": "T_P_Bus",
        "T_F_LT": "T_F_LTrk","T_F_MT": "T_F_MTrk","T_F_HT": "T_F_HTrk",
    }

    base_up = base.upper()
    if "FUELJET_INT" in base_up:
        return "T_O_JET_Int"
    elif "FUELJET" in base_up:
        return "T_O_JET"
    ...
    if "T_R_RAIL" in base_up:   return "T_F_Rail"
    if "T_P_RAIL" in base_up:   return "T_P_Rail"
    if "DSHIPP"     in base_up: return "T_F_DSHIP"
    if "ISHIPP"     in base_up: return "T_F_ISHIP"
    return None
...
def parse_attrs(tech: str, comm_out: str, comm_in: str
    ...
    elif comm_out in ("T_F_DSHIP", "T_F_ISHIP") or (comm_out.endswith("_Rail") and fuel == "Diesel"):
        ttype = "ICE"
...
def create_fuel_commodity_df(cfg):
    comm_names = ["TRANGA", "TRABIL", "TRALPG", "TRAPET", "TRADSL", "TRAJET", "TRAH2R", "TRAELC", "TRAFOL", "TRACO2"]
    regions = ["NI", "SI"]
    df = pd.DataFrame([(comm, region) for comm in comm_names for region in regions], columns=["CommName", "Region"])
    ...
    df["CTSLvl"] = df["CommName"].apply(lambda x: "DAYNITE" if x in ["TRAH2R", "TRAELC"] else "ANNUAL")
...
def create_fuel_process_df(cfg):
    tech_names = ["FTE_TRANGA", "FTE_TRABIL", "FTE_TRALPG", "FTE_TRAPET", "FTE_TRADSL", "FTE_TRAJET", "FTE_TRAH2R", "G_ELC_T_00", "FTE_TRAFOL"]
    regions = ["NI", "SI"]
    df = pd.DataFrame([(tech, region) for tech in tech_names for region in regions], columns=["TechName", "Region"])
    df["Sets"] = ""
    df.loc[0, "Sets"] = "DISTR"
    df["Tact"] = "PJ"
    df["Tcap"] = df["TechName"].apply(lambda x: "GW" if x in ["FTE_TRAH2R", "G_ELC_T_00"] else "PJa")
    df["Tslvl"] = df["TechName"].apply(lambda x: "DAYNITE" if x in ["FTE_TRAH2R", "G_ELC_T_00"] else "")
    final_column_order = cfg["Columns"]
    return df[final_column_order]

def create_commodity_df(cfg):
    comm_names = ["T_P_Car", "T_C_Car", "T_P_Mcy", "T_P_Bus", "T_F_LTrk", "T_F_MTrk", "T_F_HTrk", "T_F_Rail", "T_P_Rail", "T_F_DSHIP", "T_F_ISHIP", "T_O_JET", "T_O_JET_Int", "H2R"]
    regions = ["NI", "SI"]
    df = pd.DataFrame([(comm, region) for comm in comm_names for region in regions], columns=["CommName", "Region"])
    df["Csets"] = df["CommName"].apply(lambda x: "NRG" if x == "H2R" else "DEM")
    df["Unit"] = df["CommName"].apply(lambda x: "BVkm" if x in ["T_P_Car", "T_C_Car", "T_P_Mcy", "T_P_Bus", "T_F_LTrk", "T_F_MTrk", "T_F_HTrk"] else "PJ")
...
def create_process_df(cfg):
    tech_names_base = [
        "T_P_CICEPET15", "T_P_CICEDSL15", "T_P_CBEVNEW15", "T_P_CBEVUSD15", "T_P_CICELPG15", "T_P_CHYBPET15", "T_P_CPHEVPET15", "T_P_CPHEVELC15",
        "T_C_CICEPET15", "T_C_CICEDSL15", "T_C_CBEVNEW15", "T_C_CICELPG15", "T_C_CHYBPET15",
        "T_P_MICEPET15", "T_P_MBEVELC15",
        "T_P_BICEPET15", "T_P_BICEDSL15", "T_P_BICELPG15", "T_P_BBEVELC15",
        "T_F_LTICEPET15", "T_F_LTICEDSL15", "T_F_LTBEVELC15",
        "T_F_MTICEDSL15", "T_F_MTBEVELC15", "T_F_HTICEDSL15", "T_F_HTBEVELC15",
        "T_F_DSHIPP15", "T_F_ISHIPP15", "T_O_FuelJet", "T_O_FuelJet_Int", "T_R_Rail15", "T_P_Rail15"
    ]
    regions = ["NI", "SI"]
    exceptions = {"T_F_DSHIPP15", "T_F_ISHIPP15", "T_O_FuelJet", "T_O_FuelJet_Int", "T_R_Rail15", "T_P_Rail15"}
    tech_names = [
    ...
    df = pd.DataFrame([(tech, region) for tech in tech_names for region in regions], columns=["TechName", "Region"])
    df["Sets"] = "DMD"
    df["Tact"] = df["TechName"].apply(lambda x: "PJ" if get_base_name(x, tech_names_base) in exceptions else "BVkm")
    df["Tcap"] = df["TechName"].apply(lambda x: assign_tcap(get_base_name(x, tech_names_base)))
    final_column_order = cfg["Columns"]
    return df[final_column_order]

def create_newtech_process_df(cfg):
    tech_names = [
        "T_P_CICEPET", "T_P_CICEDSL", "T_P_CHYBPET", "T_P_CHYBDSL", "T_P_CBEVELC", "T_P_CFCH2R", "T_P_CPHEVPET", "T_P_CPHEVDSL",
        "T_C_CICEPET", "T_C_CICEDSL", "T_C_CHYBPET", "T_C_CHYBDSL", "T_C_CBEVELC", "T_C_CFCH2R", "T_C_CPHEVPET", "T_C_CPHEVDSL",
        "T_P_McyICEPET", "T_P_McyBEVELC",
        "T_F_LTrICEDSL", "T_F_LTrBEVELC", "T_F_LTrFCH2R",
        "T_F_MTrICEDSL", "T_F_MTrBEVELC", "T_F_MTrFCH2R",
        "T_F_HTICEDSL", "T_F_HTBEVELC", "T_F_HTFCH2R",
        "T_P_BusICEDSL", "T_P_BusBEVELC", "T_P_BusFCH2R"
    ]
    df = pd.DataFrame({
        "Sets": "DMD",
        "TechName": tech_names,
...
def create_fuel_process_parameters_df(cfg):
    ...
    for _, row in tech_df.iterrows():
        ...
        comm_in_list = expanded_comm_in.get(tech, [comm_out.replace("TRA", "").replace("TRAELC", "ELCD")])
...
def create_process_parameters_df(cfg: Mapping[str, list[str]]) -> pd.DataFrame:
    ...
    # Comm-Out map
    bases: list[str] = tech_df["TechName"].map(strip_level).unique().tolist()
    comm_out_map = {
            t: comm_out_for_tech(strip_level(t))
            for t in tech_df["TechName"]
        }
    ...
    for tech, region in tech_df[["TechName", "Region"]].itertuples(index=False):
        ...
        comm_ins = next((lst for key, lst in SPECIAL_COMM_IN.items() if key in tech), None)
        if comm_ins is None:
        ...
        for comm_in in comm_ins:
            ...
            row = {"TechName": tech, "Region": region, "Comm-In": comm_in, "Comm-Out": comm_out}
...
def creat_process_paramereters2_df(cfg: Mapping[str, list[str]]) -> pd.DataFrame:
    ...
    agg_df = base_df.groupby(["Region", "Comm-Out"], as_index=False)["ACT_BND~2023"].sum()
...
def main() -> None:
    tasks = [
        (create_fuel_commodity_df,
         {"Columns": ["Csets", "Region", "CommName", "Unit", "LimType", "CTSLvl", "Ctype"]},
        ...
        (create_fuel_process_parameters_df,
         {"Columns": ["TechName", "Region", "Comm-In", "Comm-Out",
                      "Share-I~UP", "Share-I~UP~2025", "Share-I~UP~2060",
                      "EFF", "Life", "FIXOM", "VAROM", "FLO_DELIV"]},
         "tra_fuel_process_parameters.csv"),
        (create_process_parameters_df,
         {"Columns": ["TechName", "Region", "Comm-In", "Comm-Out",
                      "EFF~2023", "LIFE", "ACT_BND~2023", "ACT_BND~0", "Cap2Act", "NCAP_AFA~2023", "INVCOST", "VAROM", "PRC_resid~2023", "PRC_resid~2045", "PRC_resid~2050", "Share", "Share~0", "CEFF~2023"]},
