...
def apply_productivity_penalty_on_afa(df: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    df.loc[mask, "annual_utilisation_rate_2024"] *= 1 - penalty
...
def build_baseyear_table(year: int) -> pd.DataFrame:
    ...
    annual_utilisation_2024 = vkt_utils[
        ["vehicletype", "tertile", "tertile_AFA"]
    ].rename(columns={"tertile_AFA": "annual_utilisation_rate_2024"})
    ...
    df = df.merge(annual_utilisation_2024, on=["vehicletype", "tertile"], how="left")
