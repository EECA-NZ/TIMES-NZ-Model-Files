...
official_generation_no_cogen = pd.read_csv(
    f"{STAGE_1_DATA}/mbie/mbie_ele_only_generation.csv"
)
...
emi_md = pd.read_parquet(
    f"{STAGE_1_DATA}/electricity_authority/emi_md.parquet", engine="pyarrow"
)
emi_solar = pd.read_csv(
    f"{STAGE_1_DATA}/electricity_authority/emi_distributed_solar.csv"
)
...
# add the emi data
base_year_gen_emi = base_year_gen_emi.merge(
    emi_md_per_plant_by, how="left", on="EMI_Name"
)
...
base_year_gen_cfs = base_year_gen_cfs.merge(
    capacity_factors, on=["FuelType", "GenerationType", "TechnologyCode"]
)


base_year_gen_cfs["EECA_Value"] = (
    base_year_gen_cfs["CapacityMW"] * 8.76
) * base_year_gen_cfs["CapacityFactor"]
...
base_year_gen_custom = base_year_gen_custom.merge(
...
df = (
    df.groupby(["Year", "PlantName", "Island", "Sector"])["capacity_installed_mw"]
    .sum()
    .reset_index()
)
...
df = df.rename(columns={"capacity_installed_mw": "CapacityMW", "Island": "Region"})
...
generic_generation = generic_generation.merge(
    capacity_factors, on=["FuelType", "GenerationType", "TechnologyCode"], how="left"
)
...
region_gen = (
    region_gen.groupby(["Region", "GenerationType", "FuelType"])["CapacityMW"]
    .sum()
    .reset_index()
)
...
generic_generation = generic_generation.merge(
    region_gen, how="left", on=["GenerationType", "FuelType"]
)
...
mbie_capacity.loc[mbie_capacity["Technology"] == "Other Cogen", "GenerationType"] = (
    "CHP"
)
...
mbie_capacity.loc[mbie_capacity["Technology"] == "Coal/Gas", "Technology"] = (
    "Coal"  # Rankine treatment might need to change but whatever
)

# trim columns and rename
mbie_capacity = mbie_capacity[["Technology", "GenerationType", "Value"]]

mbie_capacity = mbie_capacity.rename(
    columns={
        "Value": "MBIE_Value",
        "Technology": "FuelType",
    }
...
base_year_summary_cap = base_year_gen[
    ~((base_year_gen["TechnologyCode"] == "RNK") & (base_year_gen["FuelType"] == "Gas"))
]
...
cap_comparison = pd.merge(
    mbie_capacity, base_year_summary_cap, how="left", on=["FuelType", "GenerationType"]
)
# assess differences
...
base_year_gen = base_year_gen.merge(
    technology_assumptions, how="left", on="TechnologyCode"
)
...
base_year_gen = base_year_gen.merge(
    capacity_factors, how="left", on=["FuelType", "GenerationType", "TechnologyCode"]
)
...
eeca_mbie_plantname_concordance = eeca_fleet_data[
    ["PlantName", "FuelType", "MBIE_Name"]
]
# we need to distinguish some of these by fueltype or huntly gets double counted
base_year_gen = base_year_gen.merge(
    eeca_mbie_plantname_concordance, how="left", on=["PlantName", "FuelType"]
)
...
specific_parameters = specific_parameters.rename(
    columns={
        "Plant": "MBIE_Name",
...
genstack_avg_parameters = genstack_avg_parameters.rename(
    columns={
...
techs_to_fuels = pd.DataFrame(
    techs_to_fuels, columns=["TechName", "TechnologyCode", "FuelType"]
)

# we can add these to our main table to get mbie_concordance values for generic costs
base_year_gen = base_year_gen.merge(
    techs_to_fuels, how="left", on=["FuelType", "TechnologyCode"]
)
...
base_year_gen.to_csv(
    f"{output_location}/{output_name}", index=False, encoding="utf-8-sig"
)
...
cap_comparison.to_csv(
    f"{check_location}/check_base_year_ele_cap_calibration.csv", index=False
)
generic_generation.to_csv(
    f"{check_location}/check_ele_gen_generated_generics.csv", index=False
)
