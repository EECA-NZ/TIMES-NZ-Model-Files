...
from logger_setup import logger, h1, h2, red_text, blue_text
...
baseyear_industry = pd.read_csv(f"{output_location}/1_times_eeud_alignment_baseyear.csv")

# Assumptions

regional_splits_by_sector = pd.read_csv(f"{ASSUMPTIONS}/industry_demand/regional_splits_by_sector.csv")
regional_splits_by_fuel = pd.read_csv(f"{ASSUMPTIONS}/industry_demand/regional_splits_by_fuel.csv")
regional_splits_by_sector_and_fuel = pd.read_csv(f"{ASSUMPTIONS}/industry_demand/regional_splits_by_sector_and_fuel.csv")
...
def add_fuel_sector_default_shares(df):
    ...
    sector_fuel_shares = sector_fuel_shares.rename(columns = {"NI_Share" : "NIShareSectorFuelOverride"})

    df = pd.merge(df, sector_fuel_shares, on = ["Sector", "Fuel"], how = "left")

    # Overwriting 
    df["NIShareFuelOverride"] = df["NIShareSectorFuelOverride"].fillna(df["NIShareFuelOverride"])
...
def define_override_shares(df, group_used = group_used):
    ...
    df["OverrideShareOfGroup"] = df.groupby(["Sector", group_used])["WeightedOverride"].transform("sum")
...
def calculate_override_adjustments(df, group_used = group_used):    
    ...
    df["NIShareSectorAdjusted"] = df[["OverrideShareOfGroup", "NIShareSector"]].max(axis =1)    
    # use the override share in each group to calculate the fuel left to assign to the group from the fuels with no override
    df["LeftToAssignFuel"] = (df["NIShareSectorAdjusted"] - df["OverrideShareOfGroup"]) * df["TotalPerGroup"]
    ...
    df["StillNeedsAssigning"] = df["Value"] - (df["Value"] * df["NIShareFuelOverride"].fillna(0))
    ...
    df["StillNeedsAssigningShare"] =  df["StillNeedsAssigning"]/df.groupby(["Sector", group_used])["StillNeedsAssigning"].transform("sum")
...
def summarise_shares(df, group_used = group_used):
    ...
    df = df.groupby(["Sector", group_used, "Fuel"])[["Value", "NIValue"]].sum().reset_index()
...
def tidy_data(df): 
    ...
    df = df.melt(id_vars = id_cols , value_vars = cols_to_melt,
...
def report_adjusted_weights(df):
    ...
    df = df[["Sector", group_used, "Fuel", "NIShareSector", "NIShareSectorAdjusted", "UsesOverride"]].drop_duplicates()

    logger.warning(f"The following items for each {group_used} have had default shares adjusted so that fuel shares align with provided overrides:")

    summary = df = df[["Sector", group_used, "NIShareSector", "NIShareSectorAdjusted"]].drop_duplicates()
    for index, row in summary.iterrows():
        # logger.info(row)
        logger.warning(f"      {row['Sector']} ({row[group_used]}): Share was adjusted from {round(row['NIShareSector']*100,2)}% to {round(row['NIShareSectorAdjusted']*100,2)}% ")
...
def report_aggregate_subsector_shares(df):
    ...
    if not df_test.empty: 
        ...
        for index, row in df_test.iterrows():
            logger.warning(f"      {row['Sector']}: Share was adjusted from {round(row['DefaultShare']*100,0)}% to {round(row['AdjustedShare']*100,3)}% ")
            ...
            for fuel in fuels:
                ...
                if pd.notna(val):                                        
                    if ((val > 1) | (val < 0)):
                        ...
                        logger.warning(f"             {sector} use of {fuel} is {red_text("IMPOSSIBLE")} at {val_str}!") # this is very dramatic
...
def report_sector_fuel_shares(df):
    ...
    df = df.groupby(["Sector", "Fuel"])[["Value", "NIValue", "SIValue"]].sum().reset_index()
...
def calculate_shares(df):
    ...
    save_checks(df,"full_industrial_share_calculations.csv", "all regional share calculations")
