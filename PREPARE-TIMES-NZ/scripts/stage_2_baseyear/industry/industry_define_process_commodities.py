...
from filepaths import STAGE_2_DATA, CONCORDANCES
...
def check_missing_times_codes(df, varname):
    ...
    # report 
    if missing_count > 0: 
        logger.warning(f"Warning: the following {missing_count} '{varname}' items have no TIMES code equivalent")

        for index, row, in missing_codes.iterrows():
            logger.warning(f"        {row[varname]}")

        logger.warning(f"This will lead to issues: please check the input concordance file and ensure you have full '{varname}' coverage") 
    else:
...
def define_process_commodities(df):
    ...
    df["Process"] = df[["Sector_TIMES", "Fuel_TIMES", "Technology_TIMES", "EndUse_TIMES"]].agg("-".join, axis=1)
    df["CommodityIn"] = "IND" + df[["Fuel_TIMES"]].agg("-".join, axis=1)
    df["CommodityOut"] = df[["Sector_TIMES","Technology_TIMES", "EndUse_TIMES"]].agg("-".join, axis=1)
    ...
    df["CommodityIn"] = np.where(df["EndUse"] == "Feedstock", df["Fuel_TIMES"], df["CommodityIn"])
    ...
    df["CommodityOutTechAgnostic"] = df[["Sector_TIMES", "EndUse_TIMES"]].agg("-".join, axis=1)

    # and use it to overwrite all our commodities if the end use is space heating
    df["CommodityOut"] = np.where(df["EndUse"] == "Low Temperature Heat (<100 C), Space Heating",
