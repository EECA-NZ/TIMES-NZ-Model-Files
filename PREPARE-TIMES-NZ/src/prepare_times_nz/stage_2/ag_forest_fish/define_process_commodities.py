...
codes are stored in ag, forest, and fish concordances.
"""
...
USE_CODES = pd.read_csv(f"{AG_CONCORDANCES}/use_codes.csv")
TECH_CODES = pd.read_csv(f"{AG_CONCORDANCES}/tech_codes.csv")
SECTOR_CODES = pd.read_csv(f"{AG_CONCORDANCES}/sector_codes.csv")
FUEL_CODES = pd.read_csv(f"{AG_CONCORDANCES}/fuel_codes.csv")
...
def define_process_commodities(df: pd.DataFrame, run_tests=RUN_TESTS) -> pd.DataFrame:
    """Uses concordance defintions which map EEUD categories to TIMES codes.
    Builds process and commodity defintions based on these"""
    original_columns: List[str] = list(df.columns)

    # Attach TIMES codes
    df = add_times_codes(df, USE_CODES, "EndUse", run_tests=run_tests)
    df = add_times_codes(df, TECH_CODES, ["Sector", "Technology"], run_tests=run_tests)
    ...
    df["CommodityIn"] = "AGR" + df[["Fuel_TIMES"]].agg("-".join, axis=1)
...
def main():
    ...
    save_preprocessing(df, PREPRO_DF_NAME_STEP4, "Ag, forest, fish process data")
