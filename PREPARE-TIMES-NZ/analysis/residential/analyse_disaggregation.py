...
from prepare_times_nz.utilities.filepaths import ANALYSIS, STAGE_1_DATA, STAGE_2_DATA
...
def plot_heating_shares():
    """
    Generate a plot that looks like blah blah blah
    """
    ...
    heating_shares["Area"] = pd.Categorical(
        heating_shares["Area"], categories=region_order_flipped, ordered=True
    )
    ...
    chart = (
        ggplot(heating_shares, aes(y="FuelHeatingShare", x="Area", fill="HeatingType"))
        + geom_col()
        ...
        + labs(
            ...
            y="Share",
            fill="Heating Method",
            title="Share of heating method per region and dwelling type",
        )
...
def plot_disaggregation_results(
    category="Fuel",
    end_use="Low Temperature Heat (<100 C), Space Heating",
    label="Space Heating",
):
    """
    Generate a plot that looks like blah blah blah
    ...
    region_order_flipped = list(reversed(region_order))
    disag_demand["Area"] = pd.Categorical(
        disag_demand["Area"], categories=region_order_flipped, ordered=True
    )
    ...
    disag_demand["Fuel"] = pd.Categorical(
        disag_demand["Fuel"], categories=list(fuel_config.keys()), ordered=True
    )

    disag_demand["Value"] = disag_demand["Value"] / 1e3  # convert PJ

    chart = (
        ggplot(disag_demand, aes(y="Value", x="Area", fill=category))
        + geom_col()
        + facet_wrap("~DwellingType", scales="free_x")
        + coord_flip()
        + scale_fill_manual(values=fuel_config)
        + labs(
            x="Region", y="PJ", fill=category, title=f"2023 residential {label} demand"
        )
        + theme_minimal()
    )
...
def main():
    ...
    plot_disaggregation_results(
        "Fuel",
        end_use="Low Temperature Heat (<100 C), Water Heating",
        label="water heating",
    )
    plot_disaggregation_results(
        "Fuel", end_use="Intermediate Heat (100-300 C), Cooking", label="cooking"
    )
