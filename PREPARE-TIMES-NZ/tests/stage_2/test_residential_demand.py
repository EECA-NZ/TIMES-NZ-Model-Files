...
import pandas as pd
import pytest
from prepare_times_nz.stage_2 import residential_demand as rd
...
def test_get_residential_other_demand_basic(tmp_path):
    raw = pd.DataFrame(
        {
            "Year": [2023, 2023, 2023, 2021],
            "SectorGroup": ["Residential", "Residential", "Industrial", "Residential"],
            "EndUse": [
                "Low Temperature Heat (<100 C), Space Heating",  # excluded
                "Low Temperature Heat (<100 C), Water Heating",  # kept
                "Low Temperature Heat (<100 C), Space Heating",  # excluded (wrong sector)
                "Low Temperature Heat (<100 C), Cooking",  # wrong year
            ],
            "Value": [100, 200, 300, 400],
        }
    )
    csv_path = tmp_path / "eeud.csv"
    raw.to_csv(csv_path, index=False)

    result = rd.get_residential_other_demand(base_year=2023, eeud_file=str(csv_path))

    # Only water heating row from 2023 Residential should remain
    assert len(result) == 1
    row = result.iloc[0]
    assert row["Year"] == 2023
    assert row["SectorGroup"] == "Residential"
    assert row["EndUse"] == "Low Temperature Heat (<100 C), Water Heating"
    assert row["Value"] == 200


def test_get_residential_other_demand_no_matching_rows(tmp_path):
    raw = pd.DataFrame(
        {
            "Year": [2022],
    ...
    csv_path = tmp_path / "eeud_empty.csv"
    raw.to_csv(csv_path, index=False)

    result = rd.get_residential_other_demand(base_year=2023, eeud_file=str(csv_path))

    # Should be an empty DataFrame with same columns
    assert result.empty
    assert list(result.columns) == list(raw.columns)


def test_get_residential_other_demand_multiple_years(tmp_path):
    raw = pd.DataFrame(
        {
            "Year": [2022, 2023, 2024],
            "SectorGroup": ["Residential", "Residential", "Residential"],
            "EndUse": [
                "Low Temperature Heat (<100 C), Cooking",
                "Low Temperature Heat (<100 C), Cooking",
                "Low Temperature Heat (<100 C), Cooking",
            ],
            "Value": [10, 20, 30],
    ...
    csv_path = tmp_path / "eeud_multi.csv"
    raw.to_csv(csv_path, index=False)

    result = rd.get_residential_other_demand(base_year=2023, eeud_file=str(csv_path))

    # Only base_year row should remain
    assert len(result) == 1
    assert result["Year"].iloc[0] == 2023
    assert result["Value"].iloc[0] == 20


def test_disaggregate_other_demand_cross_join_and_scaling():
    # EEUD rows to be disaggregated
    eeud_df = pd.DataFrame(
        {
            "Year": [2023, 2023],
            "SectorGroup": ["Residential", "Residential"],
            "EndUse": ["Cooking", "Water Heating"],
            "Value": [100.0, 300.0],
        }
    )
    ...
    shares_df = pd.DataFrame(
        {
            "Area": ["Auckland", "Canterbury"],
            "DwellingType": ["Detached", "Joined"],
            "ShareOfPopulation": [0.25, 0.75],
        }
    )

    out = rd.disaggregate_other_demand(eeud_df, shares_df)

    # Share column is dropped; all others kept + new dims from shares_df
    assert "ShareOfPopulation" not in out.columns
    expected_cols = {"Year", "SectorGroup", "EndUse", "Area", "DwellingType", "Value"}
    assert expected_cols == set(out.columns)

    # Cross join => 2 EEUD rows x 2 share rows = 4
    assert len(out) == 4

    # Check scaled values (order-independent)
    cooking_vals = out.loc[out["EndUse"] == "Cooking", "Value"].sort_values().to_list()
    water_vals = (
        out.loc[out["EndUse"] == "Water Heating", "Value"].sort_values().to_list()
    )
    assert np.allclose(cooking_vals, [100.0 * 0.25, 100.0 * 0.75])
    assert np.allclose(water_vals, [300.0 * 0.25, 300.0 * 0.75])


def test_disaggregate_other_demand_preserves_totals_per_source_row():
    eeud_df = pd.DataFrame(
        {
            "Year": [2023, 2023, 2023],
            "SectorGroup": ["Residential", "Residential", "Residential"],
            "EndUse": ["Cooking", "Water Heating", "Lighting"],
            "Value": [50.0, 20.0, 30.0],
        }
    )
    shares_df = pd.DataFrame(
        {
            "Area": ["Auckland", "Wellington", "Canterbury"],
            "DwellingType": ["Detached", "Joined", "Detached"],
            "ShareOfPopulation": [0.5, 0.3, 0.2],  # sums to 1
        }
    )

    out = rd.disaggregate_other_demand(eeud_df, shares_df)

    # Sum back over (Area,DwellingType) to recover original totals
    summed = (
        out.groupby(["Year", "SectorGroup", "EndUse"], as_index=False)["Value"]
        .sum()
        .sort_values(["EndUse"])
        .reset_index(drop=True)
    )
    expected = eeud_df.sort_values(["EndUse"]).reset_index(drop=True)

    pd.testing.assert_series_equal(summed["Value"], expected["Value"])
    pd.testing.assert_frame_equal(
        summed.drop(columns=["Value"]), expected.drop(columns=["Value"])
    )


def test_disaggregate_other_demand_does_not_mutate_inputs():
    eeud_df = pd.DataFrame(
        {
            "Year": [2023],
            "SectorGroup": ["Residential"],
            "EndUse": ["Cooking"],
            "Value": [100.0],
        }
    )
    shares_df = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "DwellingType": ["Detached"],
            "ShareOfPopulation": [1.0],
        }
    )

    eeud_copy = eeud_df.copy(deep=True)
    shares_copy = shares_df.copy(deep=True)

    _ = rd.disaggregate_other_demand(eeud_df, shares_df)

    pd.testing.assert_frame_equal(eeud_df, eeud_copy)
    pd.testing.assert_frame_equal(shares_df, shares_copy)


def test_disaggregate_other_demand_with_empty_inputs_returns_empty():
    # Empty shares => empty output
    eeud_df = pd.DataFrame(
        {
            "Year": [2023],
            "SectorGroup": ["Residential"],
    ...
    shares_df = pd.DataFrame(columns=["Area", "DwellingType", "ShareOfPopulation"])

    out1 = rd.disaggregate_other_demand(eeud_df, shares_df)
    assert out1.empty

    # Empty eeud => empty output
    eeud_empty = pd.DataFrame(columns=["Year", "SectorGroup", "EndUse", "Value"])
    shares_nonempty = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "DwellingType": ["Detached"],
            "ShareOfPopulation": [1.0],
    ...
    out2 = rd.disaggregate_other_demand(eeud_empty, shares_nonempty)
    assert out2.empty
...
def test_get_burner_island_split_basic(tmp_path):
    # Model output with multiple fuels; only Gas/LPG should be included
    model_df = pd.DataFrame(
        {
            "Area": ["Auckland", "Canterbury", "Auckland", "Canterbury"],
            "Fuel": ["Gas/LPG", "Gas/LPG", "Electricity", "Electricity"],
            "Value": [100.0, 300.0, 999.0, 888.0],
        }
    )

    # Island concordance: Region -> Island, as expected by add_islands()
    ...
    path = tmp_path / "islands.csv"
    island_df.to_csv(path, index=False)

    out = rd.get_burner_island_split(model_df, island_file=str(path))

    # Columns present
    assert list(out.columns) == ["Island", "Value", "Share"]

    # Totals per island
    # NI: 100, SI: 300 → total 400 → shares 0.25, 0.75
    ni_val = out.loc[out["Island"] == "NI", "Value"].item()
    si_val = out.loc[out["Island"] == "SI", "Value"].item()
    ni_share = out.loc[out["Island"] == "NI", "Share"].item()
    si_share = out.loc[out["Island"] == "SI", "Share"].item()

    assert ni_val == 100.0
    assert si_val == 300.0
    assert np.isclose(ni_share, 0.25)
    assert np.isclose(si_share, 0.75)


def test_get_burner_island_split_missing_fuel_column_raises(tmp_path):
    model_df = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "Value": [10.0],
    ...
    pd.DataFrame({"Region": ["Auckland"], "Island": ["NI"]}).to_csv(
        tmp_path / "islands.csv", index=False
    )

    with pytest.raises(KeyError, match="Missing expected column in model output"):
        rd.get_burner_island_split(model_df, island_file=str(tmp_path / "islands.csv"))


def test_get_burner_island_split_propagates_island_merge_error(tmp_path):
    # Concordance missing 'Island' column -> add_islands should raise KeyError
    pd.DataFrame({"Region": ["Auckland"]}).to_csv(
        tmp_path / "bad_islands.csv", index=False
    )
    ...
    with pytest.raises(KeyError):
        rd.get_burner_island_split(
            model_df, island_file=str(tmp_path / "bad_islands.csv")
        )


def test_get_burner_island_split_zero_total_yields_nan_shares(tmp_path):
    model_df = pd.DataFrame(
        {
            "Area": ["Auckland", "Canterbury"],
            "Fuel": ["Gas/LPG", "Gas/LPG"],
            "Value": [0.0, 0.0],
        }
    )
    pd.DataFrame({"Region": ["Auckland", "Canterbury"], "Island": ["NI", "SI"]}).to_csv(
        tmp_path / "islands.csv", index=False
    )

    out = rd.get_burner_island_split(
        model_df, island_file=str(tmp_path / "islands.csv")
    )

    # Values are zero; shares should be NaN (0/0)
    assert (out["Value"] == 0.0).all()
    assert out["Share"].isna().all()


def test_get_burner_island_split_does_not_mutate_input(tmp_path):
    model_df = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "Fuel": ["Gas/LPG"],
    ...
    model_copy = model_df.copy(deep=True)

    pd.DataFrame({"Region": ["Auckland"], "Island": ["NI"]}).to_csv(
        tmp_path / "islands.csv", index=False
    )

    _ = rd.get_burner_island_split(model_df, island_file=str(tmp_path / "islands.csv"))

    pd.testing.assert_frame_equal(model_df, model_copy)


def test_get_burner_fuel_split_happy_path(tmp_path):
    base_year = rd.BASE_YEAR
    raw = pd.DataFrame(
        {
            "Sector": ["Residential", "Residential", "Residential", "Industrial"],
            "EndUse": [
                "Low Temperature Heat (<100 C), Space Heating",
                "Low Temperature Heat (<100 C), Space Heating",
                "Low Temperature Heat (<100 C), Water Heating",  # should be excluded
                "Low Temperature Heat (<100 C), Space Heating",  # excluded by Sector
            ],
            "Year": [base_year, base_year, base_year, base_year],
            "Fuel": ["Natural Gas", "LPG", "Natural Gas", "Natural Gas"],
            "Value": [100.0, 50.0, 999.0, 123.0],
        }
    )
    csv_path = tmp_path / "eeud.csv"
    raw.to_csv(csv_path, index=False)

    out = rd.get_burner_fuel_split(eeud_file=str(csv_path))

    # Expected: only NG and LPG from residential space heating in base_year
    expected = pd.DataFrame(
        {
            "Fuel": ["LPG", "Natural Gas"],
            "Value": [50.0, 100.0],
            "Share": [50.0 / 150.0, 100.0 / 150.0],
        }
    )
    # Sort for deterministic comparison
    out_sorted = out.sort_values("Fuel").reset_index(drop=True)
    expected_sorted = expected.sort_values("Fuel").reset_index(drop=True)

    pd.testing.assert_frame_equal(out_sorted, expected_sorted)


@pytest.mark.parametrize("missing_col", ["Sector", "EndUse", "Year", "Fuel", "Value"])
def test_get_burner_fuel_split_missing_required_column_raises(tmp_path, missing_col):
    raw = pd.DataFrame(
        {
            "Sector": ["Residential"],
            "EndUse": ["Low Temperature Heat (<100 C), Space Heating"],
    ...
    csv_path = tmp_path / "eeud_missing.csv"
    raw.to_csv(csv_path, index=False)

    with pytest.raises(KeyError, match="Missing required column"):
        rd.get_burner_fuel_split(eeud_file=str(csv_path))


def test_get_burner_fuel_split_filters_out_non_target_rows(tmp_path):
    base_year = rd.BASE_YEAR
    raw = pd.DataFrame(
        {
            ...
            "Fuel": ["Natural Gas", "Wood"],  # only first is target, second excluded
            "Value": [100.0, 200.0],
        }
    )
    csv_path = tmp_path / "eeud_filter.csv"
    raw.to_csv(csv_path, index=False)

    out = rd.get_burner_fuel_split(eeud_file=str(csv_path))

    # Should only contain Natural Gas row
    assert set(out["Fuel"]) == {"Natural Gas"}
    assert out["Value"].sum() == 100.0


def test_get_burner_fuel_split_zero_total_yields_nan_share(tmp_path):
    base_year = rd.BASE_YEAR
    raw = pd.DataFrame(
        {
            "Sector": ["Residential", "Residential"],
            "EndUse": ["Low Temperature Heat (<100 C), Space Heating"] * 2,
            "Year": [base_year, base_year],
            "Fuel": ["Natural Gas", "LPG"],
            "Value": [0.0, 0.0],
        }
    )
    csv_path = tmp_path / "eeud_zero.csv"
    raw.to_csv(csv_path, index=False)

    out = rd.get_burner_fuel_split(eeud_file=str(csv_path))

    assert np.allclose(out["Value"], [0.0, 0.0])
    assert out["Share"].isna().all()
...
def _write_island_file(tmp_path):
    conc = pd.DataFrame(
        {
            "Region": ["Auckland", "Canterbury"],
            "Island": ["NI", "SI"],
    ...
    p = tmp_path / "islands.csv"
    conc.to_csv(p, index=False)
    return str(p)


def test_distribute_burner_gas_happy_path(tmp_path):
    # Model has Gas/LPG for NI and SI plus a non-gas row
    df = pd.DataFrame(
        {
            "Area": ["Auckland", "Canterbury", "Auckland"],
            "Technology": ["Boiler", "Boiler", "Heat pump"],
            "Fuel": ["Gas/LPG", "Gas/LPG", "Electricity"],
            "Value": [100.0, 40.0, 10.0],
        }
    )
    islands = _write_island_file(tmp_path)

    out = rd.distribute_burner_gas(df, ni_lpg_share=0.30, island_file=islands)

    # Electricity preserved; Gas/LPG replaced by Natural Gas and LPG
    assert set(out["Fuel"]) == {"Natural Gas", "LPG", "Electricity"}

    # Auckland (NI): 100 -> LPG 30, NG 70
    ak = out[out["Area"] == "Auckland"]
    assert np.isclose(ak.loc[ak["Fuel"] == "LPG", "Value"].sum(), 30.0)
    assert np.isclose(ak.loc[ak["Fuel"] == "Natural Gas", "Value"].sum(), 70.0)

    # Canterbury (SI): 40 -> all LPG
    can = out[out["Area"] == "Canterbury"]
    assert np.isclose(can.loc[can["Fuel"] == "LPG", "Value"].sum(), 40.0)
    assert np.isclose(can.loc[can["Fuel"] == "Natural Gas", "Value"].sum(), 0.0)

    # Electricity row untouched
    assert np.isclose(out[out["Fuel"] == "Electricity"]["Value"].sum(), 10.0)

    # Totals preserved for Gas/LPG rows by Area
    orig = df[df["Fuel"] == "Gas/LPG"].groupby("Area")["Value"].sum()
    after = out[out["Fuel"].isin(["LPG", "Natural Gas"])].groupby("Area")["Value"].sum()
    pd.testing.assert_series_equal(after.sort_index(), orig.sort_index())


def test_distribute_burner_gas_no_gas_rows_returns_same(tmp_path):
    df = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "Technology": ["Heat pump"],
    ...
    out = rd.distribute_burner_gas(df, ni_lpg_share=0.5, island_file=islands)

    # drop Island if added, compare rest
    cmp_cols = [c for c in df.columns if c in out.columns]
    pd.testing.assert_frame_equal(
        out[cmp_cols].reset_index(drop=True), df[cmp_cols].reset_index(drop=True)
    )
    # and assert no new fuel rows were created
    assert set(out["Fuel"]) == {"Electricity"}


def test_distribute_burner_gas_does_not_mutate_input(tmp_path):
    df = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "Technology": ["Boiler"],
    ...
    _ = rd.distribute_burner_gas(df, ni_lpg_share=0.2, island_file=islands)

    pd.testing.assert_frame_equal(df, df_copy)


@pytest.mark.parametrize(
    "share, exp_lpg, exp_ng",
    [
        (0.0, 0.0, 100.0),  # all NG in NI
        (1.0, 100.0, 0.0),  # all LPG in NI
    ],
)
def test_distribute_burner_gas_ni_share_edges(tmp_path, share, exp_lpg, exp_ng):
    df = pd.DataFrame(
        {
            "Area": ["Auckland"],
            "Technology": ["Boiler"],
    ...
    out = rd.distribute_burner_gas(df, ni_lpg_share=share, island_file=islands)

    assert np.isclose(out.loc[out["Fuel"] == "LPG", "Value"].sum(), exp_lpg)
    assert np.isclose(out.loc[out["Fuel"] == "Natural Gas", "Value"].sum(), exp_ng)


def test_distribute_burner_gas_bad_concordance_raises(tmp_path):
    # Missing 'Island' column -> add_islands should raise KeyError
    bad = pd.DataFrame({"Region": ["Auckland"]})
    bad_path = tmp_path / "bad_islands.csv"
    ...
    with pytest.raises(KeyError):
        rd.distribute_burner_gas(df, ni_lpg_share=0.5, island_file=str(bad_path))
