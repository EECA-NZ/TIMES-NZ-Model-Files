...
This file describes the layout of the `PREPARE-TIMES-NZ` module. The intent is that this makes the structure - which is complex - easier to understand, interrogate, and modify. 
...
- **Stage 1** is only for reading in and standardising raw data from a wide range of sources. This should not include aggregation, merging, or other manipulation. Processes should be limited to simply cleaning and standardising data for later stages. 
- **Stage 2** compiles all information for base year (historical) data. This could combine or do modelling to get all the information required for the TIMES-NZ base year. Note that outputs should be legible by any system, rather than TIMES. 
- **Stage 3**  compiles all information for scenario or tech expansion files. For example, "future technology" processing should be done here, to later feed into TIMES subres files. Again, outputs should be legible by any system, rather than TIMES.
- **Stage 4** takes the outputs of **Stage 2** and **Stage 3** and converts these into individual tables formatted for Veda. In practice, it may also directly read assumptions files from raw data to flesh out specific components of the model. 
- **Stage 5** combines the user configuration files (processed in **Stage 0**) and the Veda-formatted data (processed in **Stage 4**) to create the excel model files used to create the TIMES-NZ model.
