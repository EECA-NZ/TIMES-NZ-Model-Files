"""ETS price sensitivity analysis charts.
...
def get_sensitivity_emissions(scenarios=None):
    """Return total emissions for the selected ETS sensitivity scenarios."""

    scenario_map = _resolve_scenario_map(scenarios)
...
def get_sensitivity_electricity_demand(scenarios=None):
    """Return total annual electricity demand for selected ETS sensitivity scenarios."""
...
def create_steady_ets_emissions_chart(scenarios=None):
    """Create emissions chart for Steady ETS price sensitivity scenarios."""

    if scenarios is None:
        scenarios = STEADY_ETS_COMPARISON_SCENARIOS

    df = get_sensitivity_emissions(scenarios=scenarios)
    return _create_sensitivity_line_chart(
        df,
        "Emissions sensitivity: Steady ETS price",
        "steady_ets_price_emissions.png",
    )


def create_steady_ets_electricity_demand_chart(scenarios=None):
    """Create electricity demand chart for Steady ETS price sensitivity scenarios."""
    ...
    return _create_sensitivity_line_chart(
        df,
        "Electricity demand sensitivity: Steady ETS price",
        "steady_ets_price_electricity_demand.png",
    )


def create_shift_ets_emissions_chart(scenarios=None):
    """Create emissions chart for Shift ETS price sensitivity scenarios."""

    if scenarios is None:
        scenarios = SHIFT_ETS_COMPARISON_SCENARIOS

    df = get_sensitivity_emissions(scenarios=scenarios)
    return _create_sensitivity_line_chart(
        df,
        "Emissions sensitivity: Shift ETS price",
        "shift_ets_price_emissions.png",
    )


def create_shift_ets_electricity_demand_chart(scenarios=None):
    """Create electricity demand chart for Shift ETS price sensitivity scenarios."""
    ...
    return _create_sensitivity_line_chart(
        df,
        "Electricity demand sensitivity: Shift ETS price",
        "shift_ets_price_electricity_demand.png",
    )


def main():
    """Write ETS price sensitivity charts."""

    ensure_sensitivity_output_dir()
    create_steady_ets_emissions_chart()
    create_steady_ets_electricity_demand_chart()
    create_shift_ets_emissions_chart()
    create_shift_ets_electricity_demand_chart()
