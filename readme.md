...
Standard TIMES input files for Veda are found in `TIMES-NZ`. These contain all input information for the modelling. They are entirely automatically generated by the `PREPARE-TIMES-NZ` module, which contains all data, input assumptions, code, and methods used to build the model and its scenarios. 
